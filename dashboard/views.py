...
def dashboard_view(request):
    ...
    requests = httpx.get("https://smart-campus-tiet-production.up.railway.app/api/get-all-future-books")
    ...
    return render(request, "library.html", {"count" : books_count, "budget" : budget, "requests" : all_requests})

def events_view(request):
    user = request.user
    if not user.is_authenticated:
        return redirect('login')
    events = httpx.get("https://smart-campus-tiet-production.up.railway.app/events-api/events-count")
    events_count = events.json()

    delete_events_list_getter = httpx.get("https://smart-campus-tiet-production.up.railway.app/events-api/get-all-events")
    ...
    if request.method == 'POST':
        if 'create_event_form' in request.POST:
            ...
            create_event = httpx.post("https://smart-campus-tiet-production.up.railway.app/events-api/add-event", json=data)

        if 'delete_event_form' in request.POST:
            delete_event_name = request.POST['delete_event']
            delete_event_data = {
                "event_name": delete_event_name
            }
            delete_event = httpx.post("https://smart-campus-tiet-production.up.railway.app/events-api/delete-event", json=delete_event_data)

    return render(request, "events.html", {"count" : events_count, "delete_event_list" : delete_events_list})
...
def attendee_list(request, event_name: str):
    try:
        response = httpx.get(f'https://smart-campus-tiet-production.up.railway.app/events-api/get-all-attendees/{event_name}')
